...
class DuckieNavEnvV3(discrete.DiscreteEnv):
                    ...
                    for node_row in range(nR):
                        for node_col in range(nC):
                            for init in range(4): # initail location good
                                state = self.encode(row, col, node_deployed, node_row, node_col, init) 

                                self.coverage = np.zeros((14,9), dtype='bool')
                                self.fill_coverage(locs[init][0], locs[init][1]) 

                                if (row == locs[init][0] and col == locs[init][1]) and (node_deployed == 0): # and (node_row == locs[init][0] and node_col == locs[init][1])
                                    isd[state] += 1

                                for a in range(nA):
                                    # defaults
                                    newrow, newcol, newnode_deployed, newnode_row, newnode_col  = row, col, node_deployed, node_row, node_col
                                    reward = -0.1
                                    done = False
                                    taxiloc = (row, col)
    ...
    def encode(self, taxirow, taxicol, node_deployed, node_row, node_col, init):
        # (14) 9, 2, 14, 9, 4
        i = taxirow
        i *= 9 
        i += taxicol
...
#         # 車上也有裝mesh
#         self.fill_coverage(taxirow, taxicol)
...
#             # deploy的位置 就是紀錄當下taxi現在的位置 
        ...
        for row in range(nR):
            for col in range(nC):
                if (self.coverage[row][col] == True) and (row!=taxirow or col != taxicol) and (row!=node_row or col != node_col):
